# Codex AI ASSISTANT CONFIG v3.0 (XML Edition)

> ⚠️ 本配置的核心行为逻辑（如反馈、音效）依赖于外部脚本，如 `$HOME/.Codex/feedback_common.md`。

The project memory and development context is defined in @.Codex/MEMORY.md.
...
<claude_configuration>
    ...
    <system_hooks>
        <hook event="on_request_received">
            ...
            <action>
              使用 Bash 工具执行命令：afplay "$HOME/.Codex/sounds/feedback_request.aiff" &
              （如果音频文件不存在，忽略错误继续处理）
          </action>
        </hook>
    </system_hooks>
        ...
        <routing_logic>
            ...
            <final_step name="Confirmation and Execution">
                <instruction>
                    ...
                    2.  然后，根据 `$HOME/.Codex/feedback_common.md` 中定义的智能确认系统，向用户确认你的计划。
    ...
    <protocols>
        <tooling_guidelines>
            <reference>详细工具调用规范请参考: `$HOME/.Codex/mcp_tooling_guide.md`</reference>
        </tooling_guidelines>
        <feedback_protocol>
            <reference>
                核心反馈规范，严格遵守: `$HOME/.Codex/feedback_common.md`。
        ...
        <context_management_protocol>
            ...
            <rule name="Global Context with @-Syntax">
                <description>对于那些体积小、全局通用、且在绝大多数任务中都需要引用的核心文件（例如：数据库 schema、全局类型定义），可以使用 `@` 语法在 `AGENTS.md` 中引用。</description>
                <caution>
                    <![CDATA[
