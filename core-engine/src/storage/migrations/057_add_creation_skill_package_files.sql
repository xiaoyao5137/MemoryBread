-- 让本地技能与 Codex / Agent Skills 目录格式兼容。
-- 用户上传的技能目录按文件清单保存；脚本仅作为数据保留，不在导入时执行。
BEGIN;
...
CREATE TABLE creation_skills_next (
    ...
    cloud_skill_id      TEXT,
    source_kind         TEXT NOT NULL CHECK (source_kind IN ('creation_history', 'bake_document', 'market', 'imported')),
