...
只提炼"技术方案 / 架构设计 / 系统设计 / 产品设计文档"。
必须是完整的设计方案，包含背景、目标、方案细节等，否则 reject。

accepted=true 时，payload schema:
{
  "title": "设计方案标题",
  "summary": "一句话概述",
  "content": "设计方案完整内容（Markdown格式）",
  "design_type": "technical|architecture|product|system|other",
...
约束:
- 必须包含设计方案的核心内容（背景、目标、方案）
- 如果只是普通文档浏览、会议记录，不是设计方案，直接 reject
- 设计证据较弱时使用 `candidate`"""
...
请提炼上述内容。要求:必须总结工作动作与结果，禁止照抄菜单词/窗口壳层词或原始 OCR 长串。"""
    ...
    def extract_bake_bundle(self, candidate: Dict[str, Any], preempt_check=None) -> Dict[str, Any]:
        ...
        if preempt_check and preempt_check():
            logger.info("bake bundle 在 design 后收到抢占信号 source_knowledge_id=%s", source_knowledge_id)
        ...
        logger.info(
            "bake bundle done source_knowledge_id=%s total_elapsed_ms=%s stage_elapsed_ms=%s degraded=%s accepted={knowledge:%s,design:%s,sop:%s}",
            source_knowledge_id,
            total_elapsed_ms,
            per_stage_ms,
            degraded,
